...
class FirewallManager(CoreSysAttributes):
    ...
    async def apply_gateway_firewall_rules(self) -> None:
        ...
        In developer mode (SUPERVISOR_DEV=1) this check is intentionally skipped:
        devcontainers run Docker-in-Docker without systemd, so the normal
        iptables-via-systemd approach fails. The gateway is still protected by
        the devcontainer host's own iptables rules applied at startup.
        See: https://github.com/home-assistant/supervisor/pull/6650
        """
        if self.sys_dev:
            _LOGGER.info(
                "Skipping gateway firewall rules in developer mode (SUPERVISOR_DEV=1)",
            )
            return
